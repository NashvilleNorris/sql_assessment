...
--*****I tried coming up with a CTE for #4 but ran out of time; I know this is way off but didn't know how to put it together so I left the 2 seperate queries above^^^ and I understand that how I answered it above isn't what you were originally looking for because I assume you want the answer as 1 query
WITH first_graders AS (
	SELECT CONCAT(a.grade_id, 'st') AS grade_level, g.name AS gender, CONCAT(pe.intensity_percent, '%') AS intensity_percent, e.name AS emotion_name, p.title AS poem_title, p.text
FROM author AS a
INNER JOIN poem AS p
ON a.id = p.author_id
...
fifth_graders AS 
	(
	SELECT CONCAT(a.grade_id, 'st') AS grade_level, g.name AS gender, e.name AS emotion_name, CONCAT(pe.intensity_percent, '%') AS intensity_percent, p.title AS poem_title, p.text
FROM author AS a
INNER JOIN poem AS p
ON a.id = p.author_id
INNER JOIN poem_emotion AS pe
